...
int e_exit(const char *txt, const char *txt1, const char *txt2) {
    printf("%s%s%s\n", txt, txt1, txt2);
    system("pause");
    return 0;
}
...
int main() {
    ...
    TCHAR MainSettings[13] = "MainSettings";
    TCHAR ClickerSettings[16] = "ClickerSettings";
    ...
    if (GetPrivateProfileString(LPCTSTR(ClickerSettings), LPCTSTR("UpdateButtonCursorPosX"), LPCTSTR(""), UpdateButtonX, 8, ini.c_str()) == 0) {
        return e_exit("INI attribute UpdateButtonCursorPosX in ", ClickerSettings, " not found!");
    }
    ...
    if (not getHandle(browserProcess)) {
        return e_exit("Process browser ", browserProcess, " not found!");
    }
    ...
    if (WinHwnd != NULL) {
        ...
        SecureZeroMemory(MainSettings, sizeof(MainSettings));
        SecureZeroMemory(ClickerSettings, sizeof(ClickerSettings));
